...
void Position_Controller::getIMU(mavlink_scaled_imu_t* imu_data)
{
	auto search = mti->last_messages.find(MAVLINK_MSG_ID_SCALED_IMU);
    if (search != mti->last_messages.end())
    {
	mavlink_scaled_imu_t imu_message;
        mavlink_msg_scaled_imu_decode(&(search->second), &imu_message);
	imu_data = &imu_message;
	
    }
    else
    {
        std::cout << "IMU Message Not found\n";
    }
}
